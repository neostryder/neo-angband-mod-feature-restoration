...
Mana is 10 and fail is 30 for all three classes. Both surviving copies land on
...
The full set also checks out. The Ranger's result, `30:10:30:50`, is identical to the
...
This was useful for buying time against something chasing you, or for sealing a room you have
...
Neither the item kind nor the command exists anywhere in 4.2.6. The
...
This port keeps one part of the upstream behavior and omits another. It never carried a
...
raises that number directly, one point per spike, capped at 7. This is the same ceiling upstream
...
this mod's cap keeps upstream's own floor instead. `skill - 28`, minimum 2%, is hard to
...
the stack-size seam that was already there. `plugin.ts` here installs a handler into
...
`manifest.json`, `class.json`, `object.json`, and `plugin.js` (built from `plugin.ts`; see
below). Any
of:
...
the committed `plugin.js` is exactly what `plugin.ts` builds today. A source edit that
...
2. Decides content vs plugin the same way the three features above did. If 4.2.6 still has
   ...
   `registry:*` capability. It is already there (`registry:command`, used by `spike-doors`) or a
